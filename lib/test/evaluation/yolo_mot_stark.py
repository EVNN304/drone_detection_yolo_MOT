...
class Tracker:
    ...
    def __init__(self, q_frame: mp.Queue, name: str, parameter_name: str, dataset_name: str,
                 run_id: int = None, display_name: str = None, result_only=False, render_cfg=None, class_names=None):
        ...
        self.frame_w, self.frame_h = 1920, 1080  # Подставьте ваши размеры кадра
        ...
        self.mot_lost_frames = 0  # Счётчик от MOT
    ...
    def _validate_with_mot(self, stark_bbox, mot_data):
        ...
        # MOT молчит или цель потеряна — доверяем STARK
        if not mot_bbox or mot_lost:
            self.consecutive_failures = 0
            self.id_mismatch_count = 0
            return True

        # Низкая уверенность MOT — игнорируем
        ...
        # 1. ID совпадает + IoU высокий — идеально
        if id_match and iou >= self.iou_threshold:
            self.consecutive_failures = 0
            return True

        # 2. ID совпадает + IoU низкий — возможно окклюзия, ждём
        if id_match and iou < self.iou_threshold:
            self.consecutive_failures += 1
            if self.consecutive_failures >= self.max_consecutive_failures:
                self.logger.info(f"⚠️ STARK: IoU низкий ({iou:.2f}) в течение {self.consecutive_failures} кадров. Сброс.")
                self.target_lost_alert = True
                return False
            else:
                self.logger.info(f"⚠️ STARK: IoU низкий ({iou:.2f}), кадр {self.consecutive_failures}/{self.max_consecutive_failures}. Ждем...")
                return True

        # 3. ID разный + IoU высокий — MOT переинициализировал, обновляем ID
        ...
        # 4. ID разный + IoU низкий — ДРУГАЯ ЦЕЛЬ
        if not id_match and iou < self.iou_threshold:
            if self.id_mismatch_count >= self.max_id_mismatches:
                ...
                self.logger.info(f"⚠️ STARK: ID не совпадает, но ждем подтверждения ({self.id_mismatch_count}/{self.max_id_mismatches})")
                self.consecutive_failures += 1
                return True
        ...
        while True:
            if not self.q_frame.empty():
                ...
                if isinstance(data, dict):
                    ...
                    mot_data = {'bbox': mot_bbox, 'conf': mot_conf, 'track_id': mot_id, 'lost': mot_lost,
                                'lost_frames': mot_lost_frames}
                else:
                ...
                if is_tracking:
                    ...
                    if stark_bbox:
                        ...
                        #              (state[0] + state[2], state[1] + state[3]), (0, 255, 0), 5)

                        # Визуализация статуса
                        # alert_text = " ⚠️ LOST!" if self.target_lost_alert or mot_lost else ""
                        # status = f'ID:{self.active_track_id} | Fail:{self.consecutive_failures}/{self.max_consecutive_failures} | MOT Lost:{mot_lost_frames}{alert_text}'
                        # cv.putText(frame_disp, status, (20, 30),
                        #            cv.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 255, 0), 1)
                        # cv.putText(frame_disp, 'Tracking!', (20, 55),
                    ...
                    # Если не трекаем, ждем новую инициализацию от MOT
                ...
                # cv.putText(frame_disp, 'Press r to reset', (20, 80),
                #            cv.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 0, 0), 1)
                # cv.putText(frame_disp, 'Press q to quit', (20, 105),
                #            cv.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 0, 0), 1)
                #
                # cv.imshow("window_stark", cv.resize(frame_disp, (900, 900)))
                # cv.waitKey(1)
