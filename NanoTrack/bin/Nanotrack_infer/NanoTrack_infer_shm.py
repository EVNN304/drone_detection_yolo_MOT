...
class NanoTrackWrapper:
    ...
    def __init__(self, q_frame: mp.Queue, name: str, parameter_name: str, dataset_name: str,
                 ...
                 config_path: str = None, snapshot_path: str = None,
    ...
    def _validate_with_mot(self, nano_bbox, mot_data):
        ...
        iou = self._calculate_iou(nano_bbox, mot_bbox)
    ...
    def _get_frame_and_data(self):
        ...
        if isinstance(item, dict):
            if self.shm_reader is None:
                if self.shm_mot_to_sot_name_0:
                    ...
                    self.logger.info(f"✅ NanoTrack: подключён к Shared Memory от MOT ({self.frame_w}x{self.frame_h})")
                else:
                    self.logger.error("❌ NanoTrack: SHM не настроен!")
                    return None, None
        ...
        # === СТАРЫЙ РЕЖИМ: Queue ===
            ...
            self.logger.error(f"❌ NanoTrack: Неизвестный формат данных: {type(item)}")
            return None, None
    ...
    def run_video(self):
        ...
        while True:
            if not self.q_frame.empty():
                ...
                self.logger.info(f"Fps_nanotrack:{fps}")  # ← ДОБАВЛЕНО: вывод FPS в логгер
                ...
                telemetry = {
                    ...
                    'mot_lost': mot_lost,
                    'bbox': self._smooth_bbox(nano_bbox) if is_tracking and nano_bbox else None,
                    'fps': fps,
                }
