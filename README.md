...
🧪 VLM классификация (ЭКСПЕРИМЕНТАЛЬНО)

    ⚠️ Внимание: Модуль VLM находится в стадии активного тестирования. Функциональность может быть нестабильной, API может измениться в будущих версиях. Используйте на свой страх и риск.

    Интегрирован Vision-Language Model Qwen2.5-VL для семантического анализа объектов:
    
        Определение типа объекта (танк/БМП/БТР/дрон/самолёт)
        Определение конкретной модели (M1 Abrams, Т-72, Байрактар TB2)
        Визуализация результатов в отдельном окне

    Текущие ограничения:
    
        Требует ~6 ГБ VRAM
        Задержка обработки: 2-5 секунд на кадр
        Рекомендуется использовать отдельную GPU (device: "cuda:1")

        📥 Модель: https://huggingface.co/Qwen/Qwen2.5-VL-3B-Instruct

        
✅ Shared Memory архитектура
Полный переход на Shared Memory для передачи кадров:

    Double buffering для исключения гонок данных
    Queue-токены для синхронизации
    Экономия ~450 мс на кадр по сравнению с Queue
    Поддержка произвольного разрешения кадра
