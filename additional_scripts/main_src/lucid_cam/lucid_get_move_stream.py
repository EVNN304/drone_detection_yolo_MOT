...
def standart_proc(dif):
    ...
    kernel2 = np.ones((32, 32), np.uint8)
    thresh = 20
...
config_exposure(device, True, 130)
# kernel = np.ones((3, 3))
with device.start_stream(1):
    ...
    while True:
        ...
        for i, contour in enumerate(сontours):
            ...
            cv.rectangle(image, (x, y), (x + w, y + h), (0, 0, 0), 6)

        t_finish = time.time()

        print(f'Middle tout = {t_middle - t_start}')
        print(f'Finish tout = {t_finish - t_start}')
        cv.imshow('frame', cv.resize(image, (1400, 1400)))
